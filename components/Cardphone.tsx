...
export function Phonecard({Device}) {
  return (
    <div className="p-4"> 
         {/* 1. เพิ่มแอนิเมชันให้ตัวการ์ด: ลอยขึ้น (hover:-translate-y-1.5) และมีเงา (hover:shadow-xl) */}
         <div className="w-full max-w-sm overflow-hidden border-[2px] border-zinc-300 rounded-xl p-4 bg-white transition-all duration-300 ease-in-out hover:-translate-y-1.5 hover:shadow-xl hover:border-zinc-500">
             <div className="w-full">
               ...
               <div className="max-h-40 overflow-x-auto flex gap-4 whitespace-nowrap w-full py-[2px] mt-2 text-sm font-prompt" style={{ scrollbarWidth: 'none' }}>
                 ...
                 <p className="flex-shrink-0">•แรม: {Device.storageRam.map((item) => `${item.ram}GB`).join(", ")}</p>
                 <p className="flex-shrink-0">•หน่วยความจำ: {Device.storageRam.map((item) => `${item.storage}GB`).join(", ")}</p>
             ...
             {/* 2. เพิ่มแอนิเมชันให้แท็ก: hover:bg-gray-200 */}
             ...
             <div className="flex justify-between items-center py-[1px]">
                 ...
                 {/* 3. เพิ่มแอนิเมชันให้ปุ่ม: hover:scale-105 hover:bg-zinc-800 */}
